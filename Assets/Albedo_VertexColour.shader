Shader "Lesson/VertexColour/Albedo" //we can find the Albedo Shader in the Lesson subFolder
{
		...
		//these run at different GPU levels in platforms
	{
		...
		#pragma surface mainColour Lambert vertex:vert
		...
		*/
		void vert(inout appdata_full v, out Input o)
		{
			UNITY_INITIALIZE_OUTPUT(Input, o);
									//type,name
			o.customColour = abs(v.normal);
			//abs is a mathmatical function getting the Absolute value
			//normal vector calculates the direction the faces are facing
		}
		...
		void mainColour(Input IN, inout SurfaceOutput o)//mainColour Function
		{
			...
			o.Albedo *= IN.customColour;
			//apply over top the colour per direction of the face
		}
		ENDCG 
	}
